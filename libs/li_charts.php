...
class Li_charts {
	...
	public function parse_stats($sites,$period,$cat) {
		...
		foreach ($sites as $site) {
			...
			#Формируем урл
			$get_url = "http://www.liveinternet.ru/stat/$url_stat/$cat.csv?{$ids}graph=csv$liperiod&total=no".$slice;

			if($cat=="searches2") $get_url = "http://www.liveinternet.ru/stat/$url_stat/searches.csv?{$ids}graph=csv$liperiod&total=no".$slice;
			...
			if($cat=="index") {
				...
				$today_url="http://pda.liveinternet.ru/stat/$url_stat/index.html?nograph=yes".$slice;
